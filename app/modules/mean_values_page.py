...
def get_monthly_data_means(monthly_data):
    monthly_data_means = monthly_data.groupby(monthly_data['date'].dt.strftime('%m')).mean()\
        .drop(['Monthly max. temperature (°C)',
               'Monthly max. wind gust (km/h)',
               'Monthly max. rain in 10 minutes (mm)',
               'Monthly max. instantaneous rain rate (mm/h)',
               'Monthly min. temperature (°C)'], axis = 1).rename(columns = {'Monthly precipitation (mm)': 'Mean monthly precipitation (mm)'})
...
fig = plot_interactive_data_by_year(monthly_data, 'Monthly mean max. temperatures (°C)', "Temperatura máxima media mensual", "T (°C)")
st.plotly_chart(fig, use_container_width=True)

fig = plot_interactive_data_by_year(monthly_data, 'Monthly mean min. temperatures (°C)', "Temperatura mínima media mensual", "T (°C)")
st.plotly_chart(fig, use_container_width=True)

fig = plot_interactive_data_by_year(monthly_data, 'Monthly mean temperature (°C)', "Temperatura media mensual", "T (°C)")
st.plotly_chart(fig, use_container_width=True)

st.markdown("## Precipitación mensual y anual")
fig = plot_interactive_data_by_year(monthly_data, 'Monthly precipitation (mm)', "Precipitación acumulada mensual", "mm")
st.plotly_chart(fig, use_container_width=True)
